...
class MrpProduction(models.Model):
    ...
    def input_data(self, first_date_start):  # Thu thập name, weight, release date, due date, mold, duration expected.
        ...
        all_info = {'name': mo_name, 'wj': mo_weight, 'rj': mo_release_date, 'dj': mo_due_date, 'mold': mo_mold,
                    'pj': mo_duration_expected}
        instance_dict = pd.DataFrame(all_info)
        instance_dict = instance_dict.sort_values(by=['dj', 'name'], ascending=True)  # Sort theo 2 biến à dj và name
    ...
    def get_tabu_structure(self, solution):
        """Takes a dict (input data)
            Returns a dict of tabu attributes(pair of jobs that are swapped) as keys and [tabu_time, move_value]"""
        dict_data = {}
        swap = []
    ...
    def update_tabu_list(self, tabu_list, best_move):
        for i in range(0, len(tabu_list)-1):
            tabu_list[len(tabu_list)-i-1][0] = tabu_list[len(tabu_list)-i-2][0]
            tabu_list[len(tabu_list)-i-1][1] = tabu_list[len(tabu_list)-i-2][1]

        tabu_list[0][0] = best_move[0]
        tabu_list[0][1] = best_move[1]
        return tabu_list
    ...
    def tabu_search(self, first_date_start, instance_dict):
        ...
        while terminate < n_terminate:
            ...
            # data_file = open(f"../../data/data_size{len(self)}_tabulist{tenure}.txt", "a")
            # if terminate == 0:
            #     data_file.truncate(0)
            # data_file.write(f"{terminate}-{best_objvalue}-{tabu_list}\n")
            # data_file.write(f"{current_solution}\n")
            # data_file.write(f"{best_solution}\n")
            # data_file.close()
            # print(terminate)
            terminate += 1
    ...
    def order_input_data(self, first_date_start):
        ...
        order_all_info = {'name': order_name, 'wj': order_weight, 'rj': order_release_date, 'dj': order_due_date,
